...
public class MysqlRowTable implements ISqlRowTable {
	...
	public MysqlRowTable() {
		this(new ReentrantReadWriteLock(true));
	}

	public MysqlRowTable(ReadWriteLock reentrantLock) {
	...
	public boolean exists() {
		...
		// SQL
		ISqlBuilder sql = newSqlBuilder();
		sql.sql("SELECT COUNT(*) FROM information_schema.tables WHERE table_name=").value(getTable()).sql(" LIMIT 1");
		return getExecutor().getCount(sql) > 0;
	}
}
