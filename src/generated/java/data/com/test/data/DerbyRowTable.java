...
public class DerbyRowTable implements ISqlRowTable {
	...
	public DerbyRowTable() {
		this(new ReentrantReadWriteLock(true));
	}

	public DerbyRowTable(ReadWriteLock reentrantLock) {
	...
	public void create() {
		...
		sql.notNull();
		sql.sql(',');
		sql.column("data").sql(" VARCHAR(200) FOR BIT DATA");
		sql.sql(',');
	...
	public boolean exists() {
		...
		// SQL
		ISqlBuilder sql = newSqlBuilder();
		sql.sql("SELECT COUNT(*) FROM SYS.SYSTABLES WHERE TABLENAME=").value(getTable());
		return getExecutor().getCount(sql) > 0;
	}
}
